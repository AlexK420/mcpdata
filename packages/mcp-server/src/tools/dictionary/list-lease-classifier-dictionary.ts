...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'get',
  httpPath: '/v1/dictionary/lease-classifier',
  operationId: '24_getLeaseClassifierDictionary_1',
};

export const tool: Tool = {
  name: 'list_lease_classifier_dictionary',
  description:
...
export const handler = async (client: Datanewton, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.dictionary.listLeaseClassifier()));
  } catch (error) {
    if (isJqError(error)) {
