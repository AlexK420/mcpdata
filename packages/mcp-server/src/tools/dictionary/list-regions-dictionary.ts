...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'get',
  httpPath: '/v1/dictionary/regions',
  operationId: '16_getRegionsDictionary_1',
};

export const tool: Tool = {
  name: 'list_regions_dictionary',
  description:
...
export const handler = async (client: Datanewton, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.dictionary.listRegions()));
  } catch (error) {
    if (isJqError(error)) {
