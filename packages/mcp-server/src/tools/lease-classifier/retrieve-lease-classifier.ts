...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'get',
  httpPath: '/v1/lease_classifier',
  operationId: '24_getLeaseClassifierDictionary',
};

export const tool: Tool = {
  name: 'retrieve_lease_classifier',
  description:
...
export const handler = async (client: Datanewton, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.leaseClassifier.retrieve()));
  } catch (error) {
    if (isJqError(error)) {
