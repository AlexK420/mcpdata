...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'get',
  httpPath: '/v1/procurement/okpd2',
  operationId: '27_getOkpd2Dictionary',
};

export const tool: Tool = {
  name: 'retrieve_okpd2_procurement',
  description:
...
export const handler = async (client: Datanewton, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.procurement.retrieveOkpd2()));
  } catch (error) {
    if (isJqError(error)) {
